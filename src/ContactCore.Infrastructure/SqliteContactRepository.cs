...
public sealed class SqliteContactRepository(SqliteConnectionFactory factory, DatabaseMigrator migrator) : IContactRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            clauses.Add("(c.given_name LIKE $search ESCAPE '\\' OR c.family_name LIKE $search ESCAPE '\\' OR c.nickname LIKE $search ESCAPE '\\' OR c.notes LIKE $search ESCAPE '\\' OR EXISTS(SELECT 1 FROM phones p WHERE p.contact_id=c.id AND p.number LIKE $search ESCAPE '\\') OR EXISTS(SELECT 1 FROM emails e WHERE e.contact_id=c.id AND e.address LIKE $search ESCAPE '\\') OR EXISTS(SELECT 1 FROM contact_notes n WHERE n.contact_id=c.id AND (n.label LIKE $search ESCAPE '\\' OR n.content LIKE $search ESCAPE '\\')))");
        ...
        return await LoadContactsAsync(
            connection,
            where + " ORDER BY c.family_name COLLATE NOCASE, c.given_name COLLATE NOCASE, c.nickname COLLATE NOCASE",
            parameters,
            cancellationToken).ConfigureAwait(false);
    }
    ...
    public async Task UpsertAsync(Contact contact, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await ExecAsync(
                ...
                ("$birthday", contact.Birthday?.ToString("yyyy-MM-dd")),
                ("$notes", contact.Notes),
                ("$favorite", contact.IsFavorite ? 1 : 0),
                ("$archived", contact.IsArchived ? 1 : 0),
                ("$created", contact.CreatedAt.ToString("O")),
                ("$updated", contact.UpdatedAt.ToString("O"))).ConfigureAwait(false);

            foreach (var table in new[]
                     {
                         "phones", "emails", "addresses", "organizations", "contact_dates", "contact_notes",
                         "contact_groups", "contact_tags"
                     })
            {
                await ExecAsync(
                    connection,
                    tx,
                    $"DELETE FROM {table} WHERE contact_id=$id;",
                    cancellationToken,
                    ("$id", contact.Id.ToString())).ConfigureAwait(false);
            }
            ...
            foreach (var phone in contact.Phones)
            {
                await ExecAsync(
                    connection,
                    tx,
                    "INSERT INTO phones(id,contact_id,label,number,kind) VALUES($id,$contact,$label,$number,$kind);",
                    cancellationToken,
                    ("$id", phone.Id.ToString()),
                    ("$contact", contact.Id.ToString()),
                    ("$label", phone.Label),
                    ("$number", phone.Number),
                    ("$kind", (int)phone.Kind)).ConfigureAwait(false);
            }
            ...
            foreach (var email in contact.Emails)
            {
                await ExecAsync(
                    connection,
                    tx,
                    "INSERT INTO emails(id,contact_id,label,address,kind) VALUES($id,$contact,$label,$address,$kind);",
                    cancellationToken,
                    ("$id", email.Id.ToString()),
                    ("$contact", contact.Id.ToString()),
                    ("$label", email.Label),
                    ("$address", email.Address),
                    ("$kind", (int)email.Kind)).ConfigureAwait(false);
            }
            ...
            foreach (var address in contact.Addresses)
            {
                await ExecAsync(
                    connection,
                    tx,
                    "INSERT INTO addresses(id,contact_id,label,street,city,region,postal_code,country) VALUES($id,$contact,$label,$street,$city,$region,$postal,$country);",
                    cancellationToken,
                    ...
                    ("$country", address.Country)).ConfigureAwait(false);
            }
            ...
            foreach (var organization in contact.Organizations)
            {
                await ExecAsync(
                    connection,
                    tx,
                    "INSERT INTO organizations(id,contact_id,name,title,department) VALUES($id,$contact,$name,$title,$department);",
                    cancellationToken,
                    ("$id", organization.Id.ToString()),
                    ("$contact", contact.Id.ToString()),
                    ("$name", organization.Name),
                    ("$title", organization.Title),
                    ("$department", organization.Department)).ConfigureAwait(false);
            }
            ...
            foreach (var date in contact.Dates)
            {
                await ExecAsync(
                    connection,
                    tx,
                    "INSERT INTO contact_dates(id,contact_id,label,date_value) VALUES($id,$contact,$label,$date);",
                    cancellationToken,
                    ("$id", date.Id.ToString()),
                    ("$contact", contact.Id.ToString()),
                    ("$label", date.Label),
                    ("$date", date.Date.ToString("yyyy-MM-dd"))).ConfigureAwait(false);
            }
            ...
            foreach (var note in contact.NoteEntries)
            {
                await ExecAsync(
                    connection,
                    tx,
                    "INSERT INTO contact_notes(id,contact_id,label,content) VALUES($id,$contact,$label,$content);",
                    cancellationToken,
                    ("$id", note.Id.ToString()),
                    ("$contact", contact.Id.ToString()),
                    ("$label", note.Label),
                    ("$content", note.Content)).ConfigureAwait(false);
            }
            ...
            foreach (var group in contact.Groups)
            {
                await ExecAsync(
                    connection,
                    tx,
                    "INSERT INTO groups(id,name) VALUES($id,$name) ON CONFLICT(name) DO NOTHING;",
                    cancellationToken,
                    ("$id", group.Id.ToString()),
                    ("$name", group.Name)).ConfigureAwait(false);
                await ExecAsync(
                    connection,
                    tx,
                    "INSERT OR IGNORE INTO contact_groups(contact_id,group_id) SELECT $contact,id FROM groups WHERE name=$name COLLATE NOCASE;",
                    cancellationToken,
                    ("$contact", contact.Id.ToString()),
                    ("$name", group.Name)).ConfigureAwait(false);
            }
            ...
            foreach (var tag in contact.Tags)
            {
                await ExecAsync(
                    ...
                    cancellationToken,
                    ("$id", tag.Id.ToString()),
                    ("$name", tag.Name)).ConfigureAwait(false);
                await ExecAsync(
                    connection,
                    tx,
                    "INSERT OR IGNORE INTO contact_tags(contact_id,tag_id) SELECT $contact,id FROM tags WHERE name=$name COLLATE NOCASE;",
                    cancellationToken,
                    ("$contact", contact.Id.ToString()),
                    ("$name", tag.Name)).ConfigureAwait(false);
            }
    ...
    private static async Task<IReadOnlyList<Contact>> LoadContactsAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using (var cmd = connection.CreateCommand())
        {
            ...
            while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
            {
                contacts.Add(new Contact
                {
                    ...
                    Birthday = reader.IsDBNull(4) ? null : DateOnly.ParseExact(reader.GetString(4), "yyyy-MM-dd"),
        ...
        foreach (var contact in contacts)
            await LoadChildrenAsync(connection, contact, cancellationToken).ConfigureAwait(false);
        return contacts;
    }

    private static async Task LoadChildrenAsync(
        SqliteConnection connection,
        Contact contact,
        CancellationToken cancellationToken)
    {
        ...
        await using (var reader = await cmd.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false))
        {
            while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
                contact.Phones.Add(new(
                    ...
                    reader.GetString(1),
                    reader.GetString(2),
                    (ContactFieldKind)reader.GetInt32(3)));
        }
        ...
        cmd.CommandText = "SELECT id,label,date_value FROM contact_dates WHERE contact_id=$id ORDER BY date_value,label;";
        await using (var reader = await cmd.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false))
        {
            while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
                contact.Dates.Add(new(
                    Guid.Parse(reader.GetString(0)),
                    reader.GetString(1),
                    DateOnly.ParseExact(reader.GetString(2), "yyyy-MM-dd")));
        }

        cmd.CommandText = "SELECT id,label,content FROM contact_notes WHERE contact_id=$id ORDER BY rowid;";
        await using (var reader = await cmd.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false))
        {
            while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        ...
        cmd.CommandText = "SELECT g.id,g.name FROM groups g JOIN contact_groups cg ON cg.group_id=g.id WHERE cg.contact_id=$id ORDER BY g.name COLLATE NOCASE;";
        await using (var reader = await cmd.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false))
        {
            while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
                contact.Groups.Add(new(Guid.Parse(reader.GetString(0)), reader.GetString(1)));
        }

        cmd.CommandText = "SELECT t.id,t.name FROM tags t JOIN contact_tags ct ON ct.tag_id=t.id WHERE ct.contact_id=$id ORDER BY t.name COLLATE NOCASE;";
        await using (var reader = await cmd.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false))
        {
            while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
                contact.Tags.Add(new(Guid.Parse(reader.GetString(0)), reader.GetString(1)));
        }
    }
